...
struct SyntheticInterviewGeneratorClient: Sendable {
    ...
    Represent the candidate's current career, not whichever legacy credit happens to contain an exact technology phrase. For broad experience, debugging, optimization, or project questions, favor the newest comparably relevant work and cover more than one recent project across the interview. Use older work when the question explicitly calls for that era or when it contributes a genuinely unique, material example that newer evidence cannot supply. Do not build the interview around one old title merely because it is recognizable.
